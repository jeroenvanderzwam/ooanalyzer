...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = FactExporterFileSystem.MyFileSystemFactory.class)
public class FactExporterFileSystem implements GFileSystem {
	 ...
	 */
	public FactExporterFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + FactExporterFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<FactExporterFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public FactExporterFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			FactExporterFileSystem fs = new FactExporterFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
